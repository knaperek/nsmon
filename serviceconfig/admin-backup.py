...
#     list_display_links = []
    list_filter = ('timestamp', 'service', 'service__server', 'retcode', 'on_time')
...
class ServiceStatusSummaryAdmin(admin.ModelAdmin):
    ...
    list_filter = ('timestamp', 'service', 'status_changed_to')
...
class TestingPlanAdmin(admin.ModelAdmin):
    ...
    list_filter = ('service__server__user', 'service', ) # TODO: ?? co s tym user-om? nejde to zapnut a vypnut pre superuser / common user
    fieldsets = (
            (None, {
...
class ServiceAdmin(admin.ModelAdmin):
    ...
    list_filter = ('service_type', 'enabled', ServerFilterOnServiceList, UserFilterOnServiceList)
...
# #     filter according to superuser status - nefunguje (prepina ked su prihlaseni naraz dvaja rozni)
#         if request.user.is_superuser:
# #            ServerAdmin.list_filter = ('enabled', 'user')
#             self.list_filter = ('enabled', 'user')
#         else:
# #             ServerAdmin.list_filter = ('enabled', )
#             self.list_filter = ('enabled', )
# 
        # auth..
        if request.user.is_superuser:
            return qs
    ...
    def get_list_display(self, request):
#         list_display = ('__unicode__', 'show_service_count', 'enabled')
        if request.user.is_superuser:
            return self.list_display + ('get_username',)
        else:
            return self.list_display
    ...
    show_service_count.admin_order_field='service_count'
